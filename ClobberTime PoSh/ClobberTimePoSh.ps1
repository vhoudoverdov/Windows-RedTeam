...
{ 
    ...
    else
    {
        ...
        {
            $FileBrowser.FileNames  | % {Timestomp-Files -Path $_ -TargetDate $global:SingularDate}
        }
        if( $RandomDateSRadioButton.Checked ) 
        {
            $FileBrowser.FileNames  | % {Timestomp-Files -Path $_}
        }
...
$Form.controls.AddRange(@($DateTypeGroupBox,$PathDescriptorTextBox,$BrowseButton,$TimestompButton,$Label1,$Label2,$Label3))
